...
func updateCIProvisionedStatusFromHostedCluster(
	...
	// Build concise version message
	versionMsg := ""
	if latestHistory.Version != "" {
		versionMsg = fmt.Sprintf(" (%s)", latestHistory.Version)
	}
	...
	case configv1.CompletedUpdate:
		...
		conditions.SetStatusCondition(&ci.Status.Conditions,
			...
			fmt.Sprintf("Provisioning completed%s", versionMsg))
	...
	case configv1.PartialUpdate:
		...
		conditions.SetStatusCondition(&ci.Status.Conditions,
			...
			fmt.Sprintf("Provisioning cluster%s", versionMsg))
