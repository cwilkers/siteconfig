...
var _ = Describe("Reconcile", func() {
	...
	It("tests that HostedClusterReconciler updates ClusterInstance Conditions", func() {
		...
		// Set initial conditions for CI to be overridden by reconciler
	...
	/*
		It("tests that ClusterInstance provisioned status condition is set to False with reason set to Failed when provisioning failed", func() {
			...
			hostedCluster := &hypershiftv1beta1.HostedCluster{
				...
				Status: hypershiftv1beta1.HostedClusterStatus{
					Conditions: []hypershiftv1beta1.HostedClusterCondition{
						{
							Type:    hypershiftv1beta1.ClusterInstallRequirementsMetHostedClusterCondition,
							Status:  corev1.ConditionTrue,
							Reason:  "ClusterInstallationStopped",
							Message: "The cluster installation stopped",
						},
						{
							Type:    hypershiftv1beta1.ClusterInstallStoppedHostedClusterCondition,
							Status:  corev1.ConditionTrue,
							Reason:  "ClusterInstallStopped",
							Message: "The installation has stopped because it failed",
						},
						{
							Type:    hypershiftv1beta1.ClusterInstallCompletedHostedClusterCondition,
							Status:  corev1.ConditionFalse,
							Reason:  "InstallationCompleted",
							Message: "The installation has completed: Cluster failed to install",
						},
						{
							Type:    hypershiftv1beta1.ClusterInstallFailedHostedClusterCondition,
							Status:  corev1.ConditionTrue,
		...
		It("tests that ClusterInstance provisioned status condition is set to Unknown with reason set to StaleConditions "+
			"when HostedCluster.Spec.Installed=true and the deployment conditions have not been updated", func() {
			key := types.NamespacedName{
				Namespace: clusterNamespace,
				Name:      clusterName,
			}
			...
			hostedCluster.Spec.Installed = true
			Expect(c.Update(ctx, hostedCluster)).To(Succeed())
